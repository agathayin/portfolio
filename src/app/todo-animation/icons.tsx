export const SunIcon = ({ ref }: { ref: React.Ref<HTMLDivElement> }) => {
  return (
    <div ref={ref} className="absolute top-6 right-6 scale-0">
...
export const CloudIcon = ({ ref }: { ref: React.Ref<HTMLDivElement> }) => {
  return (
    <div ref={ref} className="absolute top-10 left-10 w-20 h-10 bg-white rounded-full opacity-0">
      <div className="absolute w-12 h-12 bg-white rounded-full -top-4 left-6" />
    </div>
  );
};

export const CloudSmallIcon = ({ ref }: { ref: React.Ref<HTMLDivElement> }) => {
  return (
    <div ref={ref} className="absolute top-5 left-40 w-14 h-7 bg-white rounded-full opacity-0">
      <div className="absolute w-8 h-8 bg-white rounded-full -top-3 left-4" />
    </div>
  );
};

export const GrassIcon = ({ ref }: { ref: React.Ref<HTMLDivElement> }) => {
  return (
    <div
      ref={ref}
      className="absolute bottom-0 w-full h-20 bg-gradient-to-t from-green-500 to-green-300 scale-y-0 opacity-0"
    />
  );
};

export const TreeIcon = ({ ref }: { ref: React.Ref<HTMLDivElement> }) => {
  return (
    <div ref={ref} className="absolute bottom-20 left-1/2 -translate-x-1/2 opacity-0 scale-0">
      <div className="flex flex-col items-center rotate-180">
        <div className="w-6 h-12 bg-amber-700 rounded" />
        <div className="w-20 h-20 bg-green-500 rounded-full -mt-6 shadow-md" />
      </div>
    </div>
  );
};
